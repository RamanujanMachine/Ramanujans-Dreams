"""
SmallAngleSearchMod — search-stage module driving :class:`SmallAngleSearch`.

Modelled on :class:`SearcherModV1` (the modern DTO/JSONL pipeline), NOT on the
older ``GeneticSearchMod`` which predates the DTO refactor.  For each unique
shard (deduplicated by ``shard_id`` across all constants) it opens a
``worker_pool`` writing ``EXPORT_SEARCH_RESULTS/<shard_id>.jsonl`` and runs the
Small Angle hill-climb **once per identified constant**.  A constant whose
...
class SmallAngleSearchMod(SearcherModScheme):
    """Search module — per-shard, per-constant Small Angle hill-climb."""
    ...
    def __init__(self, priorities, use_LIReC: bool = True):
        ...
        super().__init__(
            priorities,
            use_LIReC,
            name="SmallAngleSearch",
            description="Search module — small-angle local hill-climb with Tier-1 DTO output",
            version="1.0.0",
        )
    ...
    def execute(self) -> None:
        ...
        for shard_id, shard in SmartTQDM(
            shard_by_id.items(),
            desc="Small-angle search in shards: ",
    ...
    def _run_shard(
        ...
        """Run the hill-climb for each identified constant of a single shard."""
        cmf_id, shard_id, shard_encoding_str = derive_cmf_and_shard_ids(shard)
        Logger(
            f"Starting Small Angle search on shard {shard_id} (cmf={cmf_id})",
            Logger.Levels.debug,
        ).log()
        ...
        eval_pool, pq_manager = make_shared_eval_pool(
            shard, start, search_config.SA_NUM_EVAL_WORKERS
        )

        try:
            with Logger.watchdog(
                f"Small Angle shard search (shard {shard_id})",
        ...
        Logger(
            f"Finished Small Angle search on shard {shard_id}",
            Logger.Levels.debug,
        ).log()
